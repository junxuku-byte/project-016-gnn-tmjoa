...
Computational drug repositioning has attracted intense methodological interest, with graph neural networks (GNNs) emerging as the predominant modeling paradigm. Most published GNN repositioning models operate on flat drug–disease association networks, conflating statistical co-occurrence patterns with mechanistic biology. We argue that drug repositioning should be reformulated as a mechanism-constrained hierarchical prediction problem — drug → target protein → signaling pathway → disease phenotype — that imposes a biological prior absent in flat networks. We construct a four-layer mechanism graph from 753 curated PubMed articles on temporomandibular joint osteoarthritis (TMJOA), supplemented with ChEMBL compound–target mappings (v34) and pathway annotations grounded in KEGG, Reactome, and domain-curated signaling modules. We report an unexpected empirical finding: a minimal homogeneous GNN trained on this mechanism graph achieves an inductive leave-drug-out AUC of 0.85 (AUPRC 0.31) — indistinguishable from a full heterogeneous attention GNN — indicating that the four-layer topology, not architectural complexity, drives inductive generalization. Graph ablation reveals that target–pathway edge randomization causes the largest performance drop (AUC 0.72, p < 0.05 vs. full graph), confirming that mechanism graph construction is the primary determinant of inductive capability. Molecular fingerprint enhancement (ECFP4) yields a further +2.5 AUC gain (0.883 → 0.908), with largest improvements on hard folds, indicating that molecular features provide complementary signal beyond graph topology. Comparison against knowledge graph embedding methods (DistMult, ComplEx, RotatE, TransE; AUC 0.80–0.83) shows that learned message passing outperforms classical KG scoring by 2–5 AUC points. Literature validation confirms 19 of 20 top TMJOA candidates, including BGJ398 — an FGFR inhibitor with no prior PubMed-indexed TMJOA-specific literature — identified through its FGF-FGFR-MMP13 mechanism path. Crucially, we demonstrate that transductive Node2Vec overestimates repositioning performance by 21 AUC points through structural information leakage, arguing for strictly inductive evaluation protocols. This work repositions computational drug repositioning from an architecture-focused to a graph-construction-focused discipline, with molecular features as an orthogonal complement to mechanism constraints.
...
All KG methods were pre-trained on the mechanism graph triples (drug–target, target–pathway, pathway–disease), then fine-tuned with a drug–disease link prediction MLP under identical LDO protocols. Hyperparameters: Adam lr=0.005, epochs=400, early stopping patience=40, class-weighted BCE loss (positive weight=3.0).
...
The RWR baseline achieved AUC 0.72 ± 0.29. Transductive Node2Vec achieved AUC 0.90 ± 0.07, but inductive Node2Vec dropped to 0.69 ± 0.17 — a 21-point gap exposing systematic overestimation in benchmarks that do not enforce inductive evaluation (Table 2).
...
Of the top 20 AttnHeteroGNN predictions for TMJOA, 19 (95%) had at least one PubMed publication connecting the drug to TMJ or temporomandibular disorders (Table 6). Evidence levels: strong (≥10 hits) for 13 drugs, moderate (3–9 hits) for 5, weak (1–2 hits) for 1, and none for 1 (BGJ398). Known positive controls (glucosamine, hyaluronic acid, NSAIDs) all returned strong evidence. Dextrose prolotherapy ranked first (score 0.53, 218 PubMed hits) — while no explicit TMJOA–dextrose training edge existed, its high literature density likely propagated through shared pathway nodes, a phenomenon we distinguish from genuinely novel hypothesis generation (see Discussion).

**Table 6. Top-10 TMJOA Predictions with Literature Evidence**
...
The dominant finding of this study — spanning architecture ablation, KG baseline comparison, and graph ablation — is that **the four-layer mechanism graph topology, not GNN architectural complexity, drives inductive generalization in drug repositioning**. A minimal homogeneous GNN with 2-dimensional node features achieves AUC 0.85, indistinguishable from a full heterogeneous attention GNN (0.85) and 2–5 points above the best KG embedding method (ComplEx, 0.83). Architecture ablation shows a strict ceiling effect: all five GNN variants, from homogeneous to attention, differ by ≤ 0.002 AUC, indicating that the information available for inductive prediction is saturated by simple message passing on the mechanism graph.
...
The 19/20 literature validation rate warrants careful interpretation. Both the training graph and the validation search draw from PubMed-indexed TMJOA literature, so a high confirmation rate for literature-supported candidates reflects mechanism-grounded associations encoded in the graph, not de novo biological discovery. Dextrose prolotherapy (218 PubMed hits, ranked first) illustrates this: while no explicit TMJOA–dextrose training edge existed, its high literature density propagated through shared pathway nodes. We distinguish **known candidate recovery** (19/20, positive-control sanity check) from **novel hypothesis generation** (BGJ398). We recommend that future benchmarks report these categories separately and employ temporally split literature or independent validation sources (ClinicalTrials.gov, DrugBank).
