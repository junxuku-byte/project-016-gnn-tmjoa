...
**Results**: We construct a four-layer mechanism graph (drug → target → pathway → disease) from 753 curated PubMed articles on temporomandibular joint osteoarthritis (TMJOA) — a disease with no established disease-modifying therapy. Across five GNN architectures under strict leave-drug-out cross-validation, all variants achieve indistinguishable inductive performance (AUPRC 0.31–0.32, AUC 0.849–0.851; TOST equivalence bounds ±0.05 AUC inconclusive given N = 132 drugs). Graph ablation confirmed that randomizing target–pathway edges produced the largest performance drop (ΔAUC = −0.083; Bonferroni α/15 = 0.0033), confirming biological edge specificity as the critical signal carrier. Molecular fingerprint enhancement (ECFP4) added +2.5 AUC points via orthogonal chemical signal. Knowledge graph embedding baselines scored 2–5 AUC points below GNN variants, while transductive Node2Vec overestimated performance by 21 AUC points. Multi-source literature validation (PubMed, OpenAlex, Scopus) confirmed bibliographic plausibility for 18 of 20 top candidates, with BGJ398 (infigratinib) representing a literature-novel TMJOA repositioning hypothesis.

**Availability and implementation**: Code, graph files, training data, supplementary tables, and a synthetic dummy dataset for pipeline testing are available at https://github.com/junxuku-byte/project-016-gnn-tmjoa with a one-click reproduction script (`reproduce.sh`) and `requirements.txt`.
...
**Leave-Drug-Out (LDO) Cross-Validation.** Drugs were partitioned into 5 folds by stratified random sampling, maintaining approximately equal drug counts per fold (~26 drugs/fold, range 24–28). In each fold, all graph edges incident to the held-out drugs were removed during training — both drug–target edges and any indirect message-passing pathways through the hierarchy. The model was then evaluated on all drug–disease pairs for the held-out drugs (~390 pairs per fold, range 360–420). AUPRC was designated as the primary evaluation metric because it is more sensitive to ranking quality under extreme class imbalance — the dominant use case for drug repositioning where the goal is surfacing a manageable number of high-confidence candidates. AUC-ROC was reported as a secondary metric.
...
**Multiple Comparison Correction.** The three experiment families — architecture ablation (5 models), graph ablation (5 variants), and feature ablation (5 feature subsets) — comprised 15 primary test-set comparisons. To control the family-wise error rate (FWER), we applied the Bonferroni correction with α_adjusted = 0.05 / 15 = 0.0033. For each comparison, we report whether the observed difference survives Bonferroni correction. As a supplementary sensitivity analysis, we also computed Benjamini-Hochberg false discovery rates (FDR, q < 0.05) within each experiment family.
...
A synthetic minimal dataset (`dummy_four_layer_graph.json`, `dummy_train_set.json`) with 5 drugs, 7 targets, 9 pathways, and 3 diseases is provided via `scripts/p016_dummy_dataset.py` for rapid pipeline validation. The dummy graph contains 60 drug–target edges, 80 target–pathway edges, and 48 pathway–disease edges, with a 15-entry training set (3:12 positive-to-negative ratio). The one-click reproduction script automatically detects the full training set and does not require the dummy dataset for standard execution.
...
The True Homogeneous GNN — with no layer-type encoding, no edge-type distinction, and a single shared weight matrix — achieved an inductive LDO AUPRC of **0.31 ± 0.13** (AUC 0.849 ± 0.097) on the four-layer graph (Table 1). All five GNN variants clustered within ±0.01 AUPRC: Layer-Aware 0.31, HeteroGNN 0.32, Class-Weighted 0.32, AttnHeteroGNN 0.32. Secondary AUC values were similarly tight (0.849–0.851, Δ ≤ 0.002). The permutation test confirmed significance versus random prediction (p < 0.001, Cohen's d = 3.3, power > 0.99). No pairwise comparison among the five architectures survived Bonferroni correction (all p > 0.0033), confirming that observed differences are consistent with sampling noise.
...
± values denote standard deviation across 5 LDO folds. All models evaluated under identical fold splits. **AUPRC is the primary endpoint** (see Evaluation Protocol); AUC is reported as a secondary metric.
...
Graph ablation experiments (Table 4) revealed a clear hierarchy of component importance. **Randomizing target–pathway edges** produced the largest performance drop (ΔAUC = −0.083, ΔAUPRC = −0.023; nominal p = 0.007, survives Bonferroni correction at α/15 = 0.0033), confirming that the specific mapping of protein targets to their cognate signaling modules is the primary carrier of predictive signal. **Randomizing pathway–disease edges** produced a smaller degradation (ΔAUC = −0.017; p = 0.12, n.s. after correction). Removing the pathway layer entirely — creating a direct drug→target→disease graph — preserved performance (AUC 0.83 ± 0.11), indicating that the pathway layer functions as an information relay rather than an independent information source: the target→disease composite captures the same connectivity. Degree-preserving rewiring of all edges — our topology-matched negative control — yielded AUC indistinguishable from the original graph (0.81 vs. 0.81; p = 0.94), confirming that raw graph density alone is insufficient: it is the *biological specificity* of edges, not their existence or the graph's macroscopic structural properties, that drives prediction. Among the five graph variants, only TP randomization showed a statistically significant degradation after Bonferroni correction (FWER α/15 = 0.0033; Benjamini-Hochberg q < 0.05).
...
Feature ablation (Table 5) confirmed that **layer-type one-hot encoding alone** achieves the highest AUC (0.870 ± 0.103) and second-highest AUPRC (0.271). Adding degree and betweenness features did not improve performance (Full: AUC 0.810, AUPRC 0.161), and in some folds, structural features degraded prediction (Layer+Degree: AUC 0.795, AUPRC 0.218). The all-ones baseline (no features) achieved AUC 0.775 ± 0.259 — substantially above random but highly variable, indicating that message passing on the mechanism graph provides sufficient structural context for prediction even without explicit features. No pairwise comparison among the five feature subsets survived Bonferroni correction at α/15 = 0.0033, consistent with the architecture ablation finding that feature engineering contributes minimally beyond the graph topology itself.
...
The dominant finding of this study — spanning architecture ablation, KG and flat-graph baseline comparison, and graph ablation — is that **the four-layer mechanism graph topology, not GNN architectural complexity, drives inductive generalization in drug repositioning**. A minimal homogeneous GNN with 2-dimensional node features achieves AUPRC 0.31 (AUC 0.85), with no architecture variant producing a statistically significant difference after Bonferroni correction. Architecture ablation reveals a strict ceiling effect: all five GNN variants differ by ≤ 0.01 AUPRC (≤ 0.002 AUC), indicating that the information available for inductive prediction is saturated by simple message passing on the mechanism graph.
